...
monitor.current_w, monitor.current_h))  # Resize the background image to fit the screen
...
while game_is_running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT or (
                event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):  # para sair, pressione o X da janela ou ESC
